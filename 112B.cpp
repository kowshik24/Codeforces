...
int main()
{
    ...
    ll a[n+1], b[n+1], c[n+1], d[n+1], e[n+1];
    ...
    for(int i=0; i<n; i++)
    {
        ll a1 = max3(a[i],a[i+1],a[i+2]);
        ll b1 = max3(b[i],b[i+1],b[i+2]);
        ll c1 = max3(c[i],c[i+1],c[i+2]);
        ll d1 = max3(d[i],d[i+1],d[i+2]);
        ll e1 = max3(e[i],e[i+1],e[i+2]);

        ll kos = min(a1,min(b1,min3(c1,d1,e1)));
...
//    sort(e,e+n);
//
//    ll kos = min(a[n-1],min(b[n-1],min(c[n-1],min(d[n-1],e[n-1]))));
